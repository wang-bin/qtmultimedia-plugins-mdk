...
Supports **Qt 5** (mediaservice plugin) and **Qt 6.8.0+** (multimedia plugin).
...
Requires Qt **6.8.0+**. Uses [mdk-sdk](https://github.com/wang-bin/mdk-sdk) via `FindMDK.cmake` (`target_link_libraries(... mdk)`).
...
Optional: `-DMDK_USE_QT_RHI_TEXTURE_POOL=ON` enables independent Qt RHI frame-slot textures with Qt 6.8.2 or later. It defaults to `OFF`; Qt 6.8.0 and 6.8.1 use the legacy shared-target path.
